...
class MainActivity : AppCompatActivity() {
    ...
    private fun clickLemonImage() {
            ...
            when (lemonadeState){
            SELECT -> {
               changeInState = SQUEEZE
                var LemonTree = LemonTree()
                lemonSize = LemonTree.pick()
                squeezeCount = 1
            }
            SQUEEZE -> {
                if(lemonSize==0) changeInState = DRINK
            }
            DRINK -> {
                changeInState = RESTART
                lemonSize = -1
            }
            RESTART -> changeInState = SELECT
        }
        lemonadeState = changeInState
        setViewElements()
    ...
    private fun showSnackbar(): Boolean {
        ...
        val squeezeEnough = getString(R.string.enough_sqeeze)
        squeezeCount++
        lemonSize--
        if (lemonSize==0) {
            Snackbar.make(
